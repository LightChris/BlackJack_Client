...
class Painter(gui.Widget):
    ...
    def box_motion(self, e):
        if self.state == 0: return
        self.overlay.fill((0, 0, 0, 0))
        pygame.draw.rect(self.overlay, app.color.value,
                         (self.pos[0], self.pos[1], e.pos[0] - self.pos[0], e.pos[1] - self.pos[1]))
        self.repaint()
...
class App(gui.Desktop):
    def __init__(self, **params):
        ...
        self.painter = Painter(width=c.rect.w - mode.rect.w - spacer * 2, height=c.rect.h - menus.rect.h - spacer * 2,
                               style={'border': 1})
